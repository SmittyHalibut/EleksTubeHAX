...
public:
  ...
  void setDigitMap(uint8_t map, bool u=true)   { digits_map = map; if (u) update(); }
  uint8_t getDigitMap()                        { return digits_map; }

  // Helper functions
  // Sets just the one digit by digit number
  void setDigit(uint8_t digit, bool u=true)  { setDigitMap(0x01 << digit, u); }
  void setAll(bool u=true)                   { setDigitMap(all_on,  u); }
  void clear(bool u=true)                    { setDigitMap(all_off, u); }
